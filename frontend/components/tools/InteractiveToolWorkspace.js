...
export default function InteractiveToolWorkspace({ tool }) {
  ...
  const handleProcess = () => {
    ...
    try {
      if (tool.slug === 'json-formatter') {
        ...
        if (parts.length !== 3) {
          throw new Error(
            'Invalid JWT format. A valid JWT consists of 3 dot-separated parts.'
          );
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <ToolActions
        onCopy={() =>
          navigator.clipboard.writeText(outputVal || inputVal)
        }
        onClear={() => {
          setInputVal('');
          setOutputVal('');
